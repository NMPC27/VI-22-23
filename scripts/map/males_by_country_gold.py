...
# get number of males with gold medal from each country by each Games in athletes_events.csv
# and write to males_by_country_gold.csv

# open atheletes_events.csv
with open('athlete_events.csv', 'r') as atheletes_events:
    # open atheletes_by_country.csv
    # open atheletes_by_country.csv
    with open('males_by_country_gold.csv', 'w') as atheletes_by_country:
        ...
        for row in reader:
            ...
            if country != 'NOC':
                ...
                if games not in atheletes_by_country_dict[country] and gender == 'M' and medal == 'Gold':
                    # add Games to atheletes_by_country_dict
                    atheletes_by_country_dict[country][games] = 1
                elif gender == 'M' and medal == 'Gold':
