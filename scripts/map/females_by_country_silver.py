...
# get number of females with silver medal from each country by each Games in athletes_events.csv
# and write to females_by_country_silver.csv

# open atheletes_events.csv
with open('athlete_events.csv', 'r') as atheletes_events:
    # open atheletes_by_country.csv
    # open atheletes_by_country.csv
    with open('females_by_country_silver.csv', 'w') as atheletes_by_country:
        ...
        for row in reader:
            ...
            if country != 'NOC':
                ...
                if games not in atheletes_by_country_dict[country] and gender == 'F' and medal == 'Silver':
                    # add Games to atheletes_by_country_dict
                    atheletes_by_country_dict[country][games] = 1
                elif gender == 'F' and medal == 'Silver':
