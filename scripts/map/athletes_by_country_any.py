...
# get number of atheletes from each country with any medal by each Games in athletes_events.csv
# and write to athletes_by_country_any.csv

# open atheletes_events.csv
with open('athlete_events.csv', 'r') as atheletes_events:
    # open atheletes_by_country.csv
    with open('athletes_by_country_any.csv', 'w') as atheletes_by_country:
        ...
        for row in reader:
            ...
            if country != 'NOC':
                ...
                if games not in atheletes_by_country_dict[country] and medal != 'NA':
