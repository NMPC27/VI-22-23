...
# get number participants from each country by each event in athletes_events.csv
# and write to athletes_by_event.csv

# open atheletes_events.csv
with open('athlete_events.csv', 'r') as atheletes_events:
    # open atheletes_by_country.csv
    # open atheletes_by_country.csv
    with open('athletes_by_event.csv', 'w') as atheletes_by_country:
        ...
        for row in reader:
            ...
            if country != 'NOC':
                ...
                if event not in atheletes_by_country_dict[country]:
